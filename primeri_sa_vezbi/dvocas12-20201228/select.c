...
int main(int argc, char **argv){
    ...
    fd_set read_set, working_set;   /* Skupovi fajl deksriptora za citanje. */
    ...
    /* Postavlja se skup fajl deskriptora za citanje. */
    FD_ZERO(&read_set);
    FD_SET(filedes[0], &read_set);
    FD_SET(STDIN_FILENO, &read_set);

    while(1){
        working_set = read_set;

        if (select(filedes[0] + 1, &working_set, NULL, NULL, NULL) < 0)
            error("select()");

        if (FD_ISSET(filedes[0], &working_set)){
