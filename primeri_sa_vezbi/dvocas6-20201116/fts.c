...
void traverse(char *file, void *data){
    ...
    char *buffer;               /* Promenljiva za putanju do fajla. */
    ...
    if (S_ISREG(stats.st_mode)){
        ...
        for (;(entry = readdir(dp)) != NULL;){
            ...
            /* Formira se putanja za tekuci fajl. */
            if ((buffer = malloc((strlen(file) + 1 + strlen(entry->d_name) + 1) * sizeof(char))) == NULL)
                error("malloc()");
            
            strcpy(buffer, file);
            strcat(buffer, "/");
            strcat(buffer, entry->d_name);
            traverse(buffer, data);

            /* Dealocira se dinamicki alocirana memorija. */
            free(buffer);
        }
